...
export function ReadingListProvider({ children }: { children: ReactNode }) {
  const { user, mode } = useUser(); const [books, setBooks] = useState<Book[]>([]); const [genres, setGenres] = useState<string[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [attempt, setAttempt] = useState(0); const [selectedGenre, setSelectedGenre] = useState<string | null>(null); const [searchText, setSearchTextState] = useState(''); const [showKidsCelebration, setShowKidsCelebration] = useState(false); const dispatch = useDispatch(); const library = useSelector((state: RootState) => state.library); const gamification = useSelector((state: RootState) => state.gamification); const seen = useRef(library.entries);
  ...
  useEffect(() => { Object.entries(library.entries).forEach(([id, entry]) => { const previous = seen.current[id]; if (previous && previous.status !== entry.status) { console.log(`Reading queue update: ${id} is now ${entry.status}`); if (entry.status === 'finished') { dispatch(recordFinished(new Date().toISOString().slice(0, 10))); if (mode === 'kids') { setShowKidsCelebration(true); setTimeout(() => setShowKidsCelebration(false), 2400); } else Alert.alert('Great work!', 'Book finished! Your streak and badges have been updated.'); } } }); seen.current = library.entries; }, [library.entries, dispatch, mode]);
